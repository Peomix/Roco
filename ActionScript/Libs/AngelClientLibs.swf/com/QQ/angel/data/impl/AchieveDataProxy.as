   ...
   public class AchieveDataProxy implements IDataProxy
   {
      ...
      protected function processXML(param1:Object) : void
      {
         ...
         while(_loc2_ < this.xml.title.length())
         {
            ...
            _loc4_ = int(this.xml.title[_loc2_].level.length());
            if(_loc4_ > 0)
            {
               _loc5_ = 0;
               while(_loc5_ < _loc4_)
               {
                  _loc6_ = new AchieveDes();
                  _loc6_.achieveId = int(this.xml.title[_loc2_].@id);
